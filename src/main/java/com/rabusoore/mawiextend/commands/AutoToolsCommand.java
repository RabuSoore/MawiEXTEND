...
public class AutoToolsCommand implements CommandExecutor, TabCompleter {

    private final MawiEXTEND plugin;

    public AutoToolsCommand(MawiEXTEND plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (!player.hasPermission("mawiextend.command.autotools")) {
            sendConfigMessage(player, "messages.no-permission");
            return true;
        }

        boolean newState = !plugin.getToggleManager().isAutoToolsEnabled(player);
        ...
        if (args.length > 0) {
            if (args[0].equalsIgnoreCase("on")) {
                ...
                player.sendMessage(ColorUtils.parse("<red>Usage: /autotools <on|off|toggle></red>"));
                return true;
            }
        }

        plugin.getToggleManager().setAutoToolsEnabled(player, newState);
        ...
        String message = plugin.getConfig().getString("messages.auto-tools-toggle", "%prefix%Auto Tools: %status%")
